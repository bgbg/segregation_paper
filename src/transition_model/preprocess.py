...
def compute_categories(
    df: pd.DataFrame, party_columns: Dict[str, str] = None
) -> pd.DataFrame:
    ...
    agudat_votes = df[party_columns.get("agudat_israel", "party_agudat_israel")].fillna(
        0
    )
    ...
    df["Abstained"] = np.maximum(
        0, df["can_vote"] - df["legal"]
    )  # Corrected: can_vote - legal

    return df


def build_station_tensors(
    df1: pd.DataFrame, df2: pd.DataFrame, target_cities: Optional[List[str]] = None
...
def prepare_transition_data(
    pair_tag: str, config_path: str = "data/config.yaml"
) -> Dict[str, np.ndarray]:
    ...
    if not pair_tag.startswith("kn") or "_" not in pair_tag:
        raise ValueError(
            f"Invalid pair tag format: {pair_tag}. Expected format: 'knXX_YY'"
        )
