...
def apply_column_mappings(
    ...
    available_mappings = {
        heb: eng for heb, eng in column_mapping.items() if heb in df.columns
    }
...
def apply_homogenic_filtering(
    ...
    if not all(col in df.columns for col in haredi_parties):
        logger.warning(
            "Cannot apply homogenic filtering - missing Haredi party columns"
        )
        return df
    ...
    logger.info(
        f"Homogenic filtering: kept {len(df_filtered)}/{len(df)} stations "
...
def filter_target_cities(
    df: pd.DataFrame, target_cities: List[str], city_mapping: Dict[str, str]
) -> pd.DataFrame:
    ...
    logger.info(
        f"City filtering: kept {len(df_filtered)}/{len(df)} stations "
...
def harmonize_election_data(
    election_num: int, config: Dict, force: bool = False
) -> pd.DataFrame:
    ...
    interim_path = (
        Path(config["paths"]["interim_data"]) / f"harmonized_{election_num}.parquet"
    )

    # Check if output already exists
    if interim_path.exists() and not force:
        logger.info(
            f"Harmonized data for election {election_num} already exists, loading..."
        )
...
def harmonize_all_elections(
    config_path: str = "data/config.yaml", force: bool = False
...
if __name__ == "__main__":
    ...
    if harmonized_data:
        ...
        logger.warning(
            "No election data was harmonized - check if raw data files exist"
        )
