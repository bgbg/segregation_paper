...
def prep_data(config_path: str = "data/config.yaml", force: bool = False, verbose: bool = False):
    ...
    try:
        ...
        logger.info(f"Homogenic filtering: {'enabled' if homogenic_enabled else 'disabled'} "
        ...
        logger.error(f"✗ Data harmonization failed: {e}")
        if verbose:
            import traceback
            traceback.print_exc()
        return 1
...
def build_tensors(pair: str, config_path: str = "data/config.yaml", force: bool = False, verbose: bool = False):
    ...
    Args:
        ...
        config_path: Path to YAML configuration file
        force: Force rebuilding even if outputs exist
        verbose: Enable verbose logging
    """
    setup_logging(verbose)
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        for scope, scope_data in data.items():
            ...
            if x1_shape[1] != K or x2_shape[1] != K:
                logger.error(f"Invalid tensor shapes for {scope}: "
        ...
        logger.error(f"✗ Tensor building failed: {e}")
        if verbose:
            import traceback
            traceback.print_exc()
        return 1
...
def fit_model(pair: str, config_path: str = "data/config.yaml", force: bool = False, verbose: bool = False):
    ...
    try:
        ...
        election_t_path = Path(config['paths']['interim_data']) / f"harmonized_{election_t}.parquet"
        election_t1_path = Path(config['paths']['interim_data']) / f"harmonized_{election_t1}.parquet"
        ...
        if fit_summary.get('status') == 'skipped':
            ...
            if 'diagnostics' in fit_summary:
                ...
                logger.info(f"Convergence: max_rhat={diag.get('max_rhat', 'N/A'):.3f}, "
            ...
            logger.info(f"Stations: {n_stations} country-wide, {len(cities_modeled)} cities")
...
def summarize(pair: str, config_path: str = "data/config.yaml", verbose: bool = False):
