...
intra-Haredi segregation (Gorelik, 2025).
...
Shas and United Torah Judaism (UTJ), indicating predominantly Haredi voters. Cities with fewer than five such boxes were
...
elections were excluded from the transition analysis. This ensured that all included boxes represented consistent
...
Four electoral categories were modeled: 1. Shas (Sephardic ultra-Orthodox party) 2. Agudat Israel (UTJ) (Ashkenazi
...
individual ballots are secret and only aggregate counts are available, a classic ecological inference scenario (Robinson
1950; King 1997). Nave methods can fall prey to the ecological fallacydrawing false conclusions about individuals from
aggregate data (Robinson 1950).
...
pattern $D$. This parsimonious approach reflects the insight that many electoral shifts are driven by one dominant
...
voters to stick with their previous choice (Campbell, Green, and Layman 2011; Clarke et al. 2004).
...
distributed random draws but exhibit correlated behavior due to social networks and demographic clustering (Wakefield
2004; Forcina and Pellegrino 2019).

### Temporal Extension

The model is applied sequentially across election pairs (1920, 2021, , 2425). Posterior means of $Z^{country}$, $D$, and
...
data-processing code are available upon request.

### Relation to Previous Study

While this analysis differs conceptually and methodologically from my earlier paper on spatial segregation (Gorelik
...
are available.  2. Reference for abstention and invalid vote analysis in Israel  placeholder citations (Diskin et al.
2005; Arian and Shamir 2008) should be verified or replaced with more recent empirical sources.  3. Sensitivity analyses
specify any tests performed with alternative priors or  values.  4. Runtime and computational resources  optional but
recommended for reproducibility.  5. Posterior predictive check metrics  describe specific diagnostics used (e.g.,
residuals, correlation of simulated vs.  observed shares).
