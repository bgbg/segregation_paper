...
If the framework applies, three observable patterns should distinguish elite-coordinated disruption from ordinary electoral volatility:
...
2. **Within-bloc direction.** Switching should occur between parties within the identity bloc rather than toward external parties — because elite directives channel voters through intra-communal networks, not toward outsiders.
3. **Rapid, synchronized recovery.** Once the coordinating directive is withdrawn, retention rates should return to baseline within one electoral cycle — the leak stops as fast as it starts, even if individual voters who already switched do not return.
