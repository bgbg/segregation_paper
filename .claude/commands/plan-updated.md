...
**Priority rule**: User edits always take precedence. When conflicts occur between edits and pre-existing text:
- Resolve to maximum extent possible with user edits overriding existing information
- Update contradicting parts automatically when context clearly indicates user intent
- Only ask user to clarify if the conflict cannot be resolved with this precedence rule

**Conflict summary (always report)**:
- Document all conflicts found and how they were resolved
- Provide specific line references for transparency
- Show which edits overrode pre-existing text and why

**If unresolvable contradictions found**:
- Show user the contradictions with specific line references
- Explain why the precedence rule cannot resolve them
- Ask user to clarify
...
**If plan is ready:**
- Summarize all conflicts found and how they were resolved (if any)
- Summarize key changes user made (brief, 2-3 bullet points)
- Confirm no unresolvable contradictions remain
- **By default** (`--proceed` is true): Automatically invoke `/plan-ok` to begin implementation, BUT ONLY IF NO ISSUES OR CONTRADICTIONS REMAIN
- **If `--no-proceed` flag is set**: State "Plan is ready. Run `/plan-ok` to begin implementation."
...
- `--proceed`: Automatically run `/plan-ok` when plan is ready and if no open issues remain, no hanging questions, no contradictions remain (default: True)
- `--no-proceed`: If set, skip automatic invocation of `/plan-ok`
