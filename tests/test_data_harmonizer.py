...
def test_compute_voter_categories(sample_raw_election_data, column_mappings):
    ...
    pd.testing.assert_series_equal(
        actual_abstained, 
        np.maximum(0, expected_abstained),
        check_names=False
    )
...
def test_apply_homogenic_filtering(sample_harmonized_data):
    ...
    if len(df_filtered) > 0:
        haredi_fraction = (df_filtered['A_shas'] + df_filtered['B_agudat']) / df_filtered['legal']
        assert (haredi_fraction > 0.75).all(), "Some stations don't meet homogenic threshold"
    ...
    assert len(df_filtered_low) >= len(df_filtered), "Lower threshold should include more stations"
    
    # With very low threshold, should have >40% Haredi votes
    if len(df_filtered_low) > 0:
        haredi_fraction_low = (df_filtered_low['A_shas'] + df_filtered_low['B_agudat']) / df_filtered_low['legal']
        assert (haredi_fraction_low > 0.4).all(), "Some stations don't meet low threshold"
...
def test_filter_target_cities(sample_harmonized_data, city_mappings):
    ...
    df_filtered = filter_target_cities(sample_harmonized_data, target_cities, city_mappings)
    ...
    for city in city_names:
        assert any(target in city_mappings.get(city, '').lower() or 
                  city.lower() in target_cities for target in target_cities), \
...
def test_category_consistency():
    ...
    total_computed = (df_result['A_shas'] + df_result['B_agudat'] + 
                     df_result['Other'] + df_result['Abstained'])
...
def test_load_mappings_file_format():
    ...
    try:
        ...
        assert 'jerusalem' in city_mapping.values() or 'Jerusalem' in city_mapping.values()
