...
def test_build_hierarchical_model_structure(sample_tensor_data):
    ...
    if cities:
        assert len(city_vars) > 0, "Should have city-level variables when cities present"
...
def test_model_with_country_only():
    ...
    country_only_data = {
        'country': {
            ...
            'x2': np.random.multinomial(520, [0.22, 0.13, 0.57, 0.08], 50).astype(float),
...
def test_model_data_shapes(sample_tensor_data):
    ...
    data_small = {
        'country': {
            ...
            'x2': np.random.multinomial(320, [0.22, 0.13, 0.57, 0.08], 10).astype(float),
            'n1': None,
            'n2': None
        }
    }
    ...
    data_large = {
        'country': {
            ...
            'x2': np.random.multinomial(820, [0.22, 0.13, 0.57, 0.08], 200).astype(float),
            'n1': None,
            'n2': None
        }
    }
...
def test_sampling_parameter_validation():
    ...
    model = build_hierarchical_model({'country': {
